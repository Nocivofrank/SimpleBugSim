...
def update_plot():
    ...
    with lock:
        if len(data_time) > 0:
            ...
            bugs_curve.setData(data_time, data_bugcount)
            radius_curve.setData(data_time, data_radius_avg)
            immortal_curve.setData(data_time, data_immortal)
            # Auto-scroll to keep last few seconds visible
            if data_time[-1] > 20:  # start scrolling after 20s
...
def run_sim():
    ...
    class bug:
        ...
        def update(self, dt, universe_energy, bugs):
            decay = 0
            decay += self.initial_radius * (.0001 * self.radius) * dt
    ...
    while running:
        ...
        with lock:
            ...
            data_radius_avg.append(total_bug_rad/len(bugs))
